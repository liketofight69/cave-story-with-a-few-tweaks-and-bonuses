...
std::unique_ptr<Map> Map::createTestMap(Graphics& graphics)
{
    ...
    const units::Tile row{11};//11
    for (units::Tile col = 0; col < num_cols; ++col) {//col = 0
        map->tiles_[row][col] = tile;
    }//group of blocks on the left
    ...
    //Now that the four block are removed need to make scrolling camera
     map->tiles_[11][0] = tile;
    map->tiles_[10][0] = tile;//these represent the blocks on the floor
    map->tiles_[9][0] = tile;
    map->tiles_[8][0] = tile;
    map->tiles_[7][0] = tile;
    map->tiles_[6][0] = tile;
    ...
    map->tiles_[4][0] = tile;
    map->tiles_[3][0] = tile;
    //the four blocks on the right
    //HIGER THE X NUMBER IS FOR LOWER BLOCKS


    map->tiles_[14][19] = tile;
    map->tiles_[13][19] = tile;
    map->tiles_[12][19] = tile;
    map->tiles_[11][19] = tile;
    map->tiles_[10][19] = tile;
    map->tiles_[9][19] = tile;
    map->tiles_[8][19] = tile;
    map->tiles_[7][19] = tile;
    map->tiles_[6][19] = tile;
    map->tiles_[5][19] = tile;
    map->tiles_[4][19] = tile;
    map->tiles_[3][19] = tile;
    map->tiles_[2][19] = tile;
    map->tiles_[1][19] = tile;
    map->tiles_[0][19] = tile;
